...
class FrequencyCallbacks:
    ...
    def on_message(self, topic, payload):
        ...
        try:
            ...
            debug_logger(message="✅ The frequency settings did synchronize!", **_get_log_args())
    ...
    def _update_start_stop_from_center_span(self):
        ...
        if self.state.center_freq is not None and self.state.span_freq is not None:
            if self.state.span_freq <= 0:
                debug_logger(message=f"❌ Error: Frequency span cannot be zero or negative. Value received: {self.state.span_freq}", **_get_log_args())
                if app_constants.global_settings['debug_enabled']:
                    debug_logger(message=f"🟡 Warning! Invalid span value ({self.state.span_freq}) received. Ignoring update.",
                                  **_get_log_args()
    ...
    def _update_center_and_span_from_start_stop(self):
        ...
        if self.state.start_freq is not None and self.state.stop_freq is not None:
            if self.state.start_freq < 0 or self.state.stop_freq < 0:
                debug_logger(message=f"❌ Error: Start and stop frequencies cannot be negative. Start: {self.state.start_freq}, Stop: {self.state.stop_freq}.", **_get_log_args())
                if app_constants.global_settings['debug_enabled']:
                    debug_logger(message=f"🟡 Warning! Invalid negative frequency values received. Ignoring update.",
                                  **_get_log_args()
            ...
            if self.state.stop_freq < self.state.start_freq:
                debug_logger(message=f"❌ Error: Stop frequency ({self.state.stop_freq}) cannot be less than start frequency ({self.state.start_freq}).", **_get_log_args())
                if app_constants.global_settings['debug_enabled']:
                    debug_logger(message=f"🟡 Warning! Invalid start/stop combination received. Ignoring update.",
                                  **_get_log_args()

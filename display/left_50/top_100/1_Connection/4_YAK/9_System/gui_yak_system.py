...
class GenericInstrumentGui(ttk.Frame):
    ...
    def __init__(self, parent, config=None, *args, **kwargs):
        ...
        super().__init__(parent) # Pass only parent for debugging
        current_function_name = inspect.currentframe().f_code.co_name
        self.current_class_name = self.__class__.__name__

        if app_constants.LOCAL_DEBUG_ENABLE:
            debug_log(
                message=f"🟢️️️🟢 ➡️➡️ {current_function_name} to initialize the GenericInstrumentGui.",
                **_get_log_args()
            )
    ...
    def _on_tab_selected(self, *args, **kwargs):
        ...
        if app_constants.LOCAL_DEBUG_ENABLE:
            debug_log(
                    message=f"🖥️🟡 Initializing GUI for {module_name}.",
                    **_get_log_args()
                )

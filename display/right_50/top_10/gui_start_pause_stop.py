...
class BaseGUIFrame(ttk.Frame):
    ...
    def __init__(self, parent, mqtt_util, *args, **kwargs):
        ...
        try:
            ...
            # --- New Orchestration Buttons Section ---
            self.start_button = ttk.Button(
                self,
                text="START",
                command=self._start_orchestration
            )
            ...
            self.pause_button = ttk.Button(
                self,
                text="PAUSE",
                command=self._pause_orchestration
            )
            ...
            self.resume_button = ttk.Button(
                self,
                text="RESUME",
                command=self._resume_orchestration
            )
            ...
            self.subscription_label = ttk.Label(self, textvariable=self.mqtt_topic_var)
    ...
    def _start_orchestration(self):
        """Publishes the 'START' command to the orchestration topic."""
        topic = "OPEN-AIR"
        subtopic = "orchestration"
        value = "START"
        self.mqtt_util.publish_message(topic=topic, subtopic=subtopic, value=value)
        console_log(f"🚀 Published command: {topic}/{subtopic} with value: {value}")
        
    def _pause_orchestration(self):
        """Publishes the 'PAUSE' command to the orchestration topic."""
        topic = "OPEN-AIR"
        subtopic = "orchestration"
        value = "PAUSE"
        self.mqtt_util.publish_message(topic=topic, subtopic=subtopic, value=value)
        console_log(f"⏸️ Published command: {topic}/{subtopic} with value: {value}")

    def _resume_orchestration(self):
        """Publishes the 'RESUME' command to the orchestration topic."""
        topic = "OPEN-AIR"
        subtopic = "orchestration"
        value = "RESUME"
        self.mqtt_util.publish_message(topic=topic, subtopic=subtopic, value=value)

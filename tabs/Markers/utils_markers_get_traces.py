# tabs/Markers/utils_markers_get_traces.py
#
...
#
#
# Version 20250814.000100.1 (FIXED: Consolidated duplicate trace retrieval functions into a single, reusable function and centralized the trace processing logic.)

current_version = "20250814.000100.1"
current_version_hash = (20250814 * 100 * 1) # Note to self: Removed leading zeros in the hash
...
def _process_trace_data(raw_data_string, start_freq_hz, end_freq_hz, console_print_func):
    ...
    try:
        ...
        debug_log(f"Failed to process trace data string. Error: {e}",
                  file=f"{os.path.basename(__file__)} & {current_version}",
                  version=current_version,
                  function=current_function)
        return None
...
def get_marker_traces(app_instance, showtime_tab_instance, console_print_func, center_freq_hz, span_hz, device_name):
    # Function Description:
    # Gets all three traces and updates the plots on the scan monitor tab.
    current_function = inspect.currentframe().f_code.co_name
    debug_log(f"Entering {current_function}. Retrieving traces for marker display. Oh, this is going to be good!",
              file=f"{os.path.basename(__file__)} & {current_version}",
              version=current_version,
              function=current_function)
    ...
    trace_1_df = get_trace_data_by_number(app_instance, console_print_func, 1, start_freq_hz, end_freq_hz)
    trace_2_df = get_trace_data_by_number(app_instance, console_print_func, 2, start_freq_hz, end_freq_hz)
    trace_3_df = get_trace_data_by_number(app_instance, console_print_func, 3, start_freq_hz, end_freq_hz)

    # Now, update the monitor plots
    if trace_1_df is not None:
        update_top_plot(showtime_tab_instance.app_instance.scan_monitor_tab, trace_1_df, start_freq_mhz, end_freq_mhz, f"{plot_title} - Trace 1")
    if trace_2_df is not None:
        update_medium_plot(showtime_tab_instance.app_instance.scan_monitor_tab, trace_2_df, start_freq_mhz, end_freq_mhz, f"{plot_title} - Trace 2")
    if trace_3_df is not None:
        update_bottom_plot(showtime_tab_instance.app_instance.scan_monitor_tab, trace_3_df, start_freq_mhz, end_freq_mhz, f"{plot_title} - Trace 3")
        
    console_print_func(f"✅ Traces retrieved and plots updated for '{device_name}'.")
...
#
#
# Version 20250812.232900.2 (REFACTORED: Moved data loading and formatting logic to utils_showtime.py.)

current_version = "20250812.232900.2"
current_version_hash = (20250812 * 232900 * 2)
...
# NEW: Import the refactored utility functions
from tabs.Markers.utils_markers_showtime import format_hz, load_markers_data
...
class ShowtimeTab(ttk.Frame):
    ...
    def _perform_peak_search_task(self, devices, name):
        ...
        self.app_instance.after(0, self.load_markers_data) # Reload data to get fresh peak values
    ...
    def _on_tab_selected(self, event):
        ...
        if self.selected_zone:
            # NEW: Automatically trigger the peak search for the default selected zone on tab load.
            self.console_print_func(f"▶️ Automatically starting peak search for default zone: '{self.selected_zone}'.")
            self._on_zone_button_click(self.selected_zone)
        else:
        ...
        if self.app_instance and self.app_instance.inst:
            try:
                ...
                if self.selected_device_freq is not None and self.is_loop_running:
                    self.after_cancel(self.marker_trace_loop_job)
                    # CORRECTED: _start_device_trace_loop
        ...
        if self.app_instance and self.app_instance.inst:
            try:
                ...
                try:
                    ...
                    self.loop_counter_var.set(0)
                    # CORRECTED: _start_device_trace_loop
                    self._start_device_trace_loop(center_freq_hz=freq_hz, span_hz=span_hz)
                except (ValueError, TypeError) as e:

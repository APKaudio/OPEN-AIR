...
current_version = "20250803.0133.1" # this variable should always be defined below the header to make the debugging better
current_version_hash = 20250803 * 133 * 1 # Example hash, adjust as needed
...
from src.gui_elements import _print_plot_ascii # Import the ASCII art function

class TAB_PLOTTING_PARENT(ttk.Frame):
    """
    A parent tab for Plotting-related functionalities, containing child tabs
    for single scan plotting, averaging from folder, and 3D scans over time.
    """
    def __init__(self, parent_notebook, app_instance, console_print_func, style_obj=None):
        # Initializes the TAB_PLOTTING_PARENT frame and its child notebook.
        # It sets up the UI for various plotting functionalities.
        #
        # Inputs:
        ...
        #   3. Creates `self.child_notebook` to hold the plotting-specific sub-tabs.
        #   4. Instantiates `PlottingTab`, `AveragingTab`, and `ThreeDPlottingTab`.
        ...
        #
        # Outputs:
        #   None. Initializes the Plotting parent tab UI.
        #
        # (2025-07-31) Change: Initial creation, refactored from main_app.py.
        #                      Implemented child notebook and added single scan and averaging tabs.
        # (2025-07-31) Change: Added ThreeDPlottingTab.
        ...
        # (2025-08-03) Change: Added _on_parent_tab_selected to display ASCII art for Plotting tab.
        ...
        debug_log(f"Initializing Plotting Parent Tab.",
                    file=f"{__file__} - {current_version}",
                    version=current_version,
                    function=current_function)

        # Create a notebook for child tabs within the Plotting parent tab
        self.child_notebook = ttk.Notebook(self, style='PlottingChild.TNotebook')
        self.child_notebook.pack(expand=True, fill="both", padx=5, pady=5)

        # Instantiate child tabs and add them to the notebook
        self.single_scan_plotting_tab = PlottingTab(self.child_notebook, self.app_instance, self.console_print_func, style_obj=self.style_obj)
        self.child_notebook.add(self.single_scan_plotting_tab, text="Single Scan Plotting")

        self.averaging_tab = AveragingTab(self.child_notebook, self.app_instance, self.console_print_func, style_obj=self.style_obj)
        self.child_notebook.add(self.averaging_tab, text="Averaging from Folder")

        self.three_d_plotting_tab = Plotting3DTab(self.child_notebook, self.app_instance, self.console_print_func, style_obj=self.style_obj)
        ...
        debug_log(f"Plotting Parent Tab initialized with child tabs. Version: {current_version}. Ready to visualize!",
                    file=f"{__file__} - {current_version}",
                    version=current_version,
                    function=current_function)

    def _on_tab_selected(self, event):
        # Handles tab change events within the child notebook of the Plotting tab.
    ...
    def _on_parent_tab_selected(self, event):
        # Function Description: Handles the event when the Plotting parent tab is selected in the main notebook.
        #
        # Inputs to this function:
        #   event (tkinter.Event): The event object that triggered the tab selection.
        #
        # Process of this function:
        #   1. Logs a debug message indicating the Plotting parent tab has been selected.
        #   2. Calls the `_print_plot_ascii` function to display the Plotting-specific ASCII art in the console.
        ...
        current_function = inspect.currentframe().f_code.co_name
        debug_log(f"Plotting Parent Tab selected. Version: {current_version}. Time to visualize some data! 📈",
                    file=f"{__file__} - {current_version}",
                    version=current_version,
                    function=current_function,
                    special=True)
        _print_plot_ascii(self.console_print_func)

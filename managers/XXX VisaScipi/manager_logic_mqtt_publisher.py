...
class VisaGuiPublisher:
    ...
    def _update_found_devices_gui(self, resources):
        ...
        try:
            base_topic = "OPEN-AIR/Device/Instrument_Connection/Search_and_Connect/Found_devices"
            ...
            for i in range(num_resources_to_show + 1, MAX_GUI_DEVICE_SLOTS + 1):
                ...
                self.mqtt_util.get_client_instance().publish(topic=f"{option_topic_prefix}/label_inactive", payload=payload_empty_label, qos=0, retain=False)
            ...
            debug_logger(message="💳 ✅ GUI device list updated with search results (up to 40 slots used).", **_get_log_args())
        except Exception as e:
            debug_logger(message=f"💳 ❌ Error in _update_found_devices_gui: {e}", **_get_log_args())
    ...
    def _publish_proxy_status(self, status: str):
        ...
        debug_logger(message=f"💳 MQTT: Published Proxy Status '{status}' to '{topic}'", **_get_log_args())
...
class VisaDeviceSearcher:
    ...
    def search_resources(self):
        ...
        if not expected_devices:
            debug_logger(message="💳 🟡 No expected devices configured in connection_yak.json. Returning all found resources.", **_get_log_args())
            self.last_search_results = all_resources # Store results
            return all_resources

        debug_logger(message=f"💳 🔍 Validating {len(all_resources)} resources against {len(expected_devices)} expected device patterns.", **_get_log_args())
        ...
        for resource_name in all_resources:
            ...
            for device_spec in expected_devices:
                ...
                if pattern and re.match(pattern, resource_name):
                    debug_logger(message=f"💳 ✅ Resource '{resource_name}' matched expected device pattern: '{pattern}'.", **_get_log_args())
                    validated_resources.append(resource_name)
                    is_valid = True
                    break
            if not is_valid:
                debug_logger(message=f"💳 ❌ Resource '{resource_name}' did not match any expected device pattern.", **_get_log_args())
        
        if not validated_resources:
            debug_logger(message="💳 🟡 No valid resources found matching any expected device patterns.", **_get_log_args())

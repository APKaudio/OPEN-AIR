...
try:
    ...
    print("Warning: 'workers.logger' not found. Using dummy logger for manager_visa_Gateway.")
    ...
    for ip in gateway_ips:
        ...
        try:
            with urllib.request.urlopen(full_url, timeout=HTTP_TIMEOUT) as response:
                ...
                for m in matches:
                    m = m.strip()
                    if "COM" not in m: targets.append(m) # Filter out COM ports, which are usually local serial
        except Exception as e: 
            debug_logger(f"   ❌ Error scraping gateway {ip}: {e}", **_get_log_args(), level="ERROR")
            
        debug_logger(f"   ✅ Found {len(targets)} resources from gateway {ip}: {targets}", **_get_log_args())
        for t in targets:

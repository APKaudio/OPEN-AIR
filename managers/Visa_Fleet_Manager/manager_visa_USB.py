...
try:
    ...
    print("Warning: 'workers.logger' not found. Using dummy logger for manager_visa_USB.")
...
def discover_usb_devices(resource_manager):
    ...
    try:
        ...
        debug_logger(f"   ✅ Found {len(usb_resources)} USB/local resources: {usb_resources}", **_get_log_args())
    except Exception as e:
        debug_logger(f"   ❌ Error scanning USB/Local Bus: {e}", **_get_log_args(), level="ERROR")

...
def get_gateway_inventory(ip):
    ...
    try:
        with urllib.request.urlopen(full_url, timeout=HTTP_TIMEOUT) as response:
            ...
            for m in matches:
                m = m.strip()
                if "COM" not in m: targets.append(m)
...
def main():
    ...
    if dedicated_ips: print(f"   ✅ Found Dedicated: {dedicated_ips}")
    if gateway_ips:   print(f"   ✅ Found Gateways: {gateway_ips}")
    if not dedicated_ips and not gateway_ips: print(f"   ⚪ No network devices found.")

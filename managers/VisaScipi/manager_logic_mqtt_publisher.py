...
class VisaGuiPublisher:
    ...
    def _update_found_devices_gui(self, resources):
        ...
        try:
            base_topic = "OPEN-AIR/Device/instrument/active/Instrument_Connection/Search_and_Connect/Found_devices"
            ...
            for i in range(1, num_resources_to_show + 1):
                ...
                self.mqtt_util.get_client_instance().publish(topic=f"{option_topic_prefix}/active", payload="true", qos=0, retain=False)
            ...
            for i in range(num_resources_to_show + 1, MAX_GUI_DEVICE_SLOTS + 1):
                ...
                self.mqtt_util.get_client_instance().publish(topic=f"{option_topic_prefix}/active", payload="false", qos=0, retain=False)
                self.mqtt_util.get_client_instance().publish(topic=f"{option_topic_prefix}/label_active", payload="", qos=0, retain=False)
                self.mqtt_util.get_client_instance().publish(topic=f"{option_topic_prefix}/label_inactive", payload="", qos=0, retain=False)
            
            if resources:
                first_device_topic = f"{base_topic}/options/1/selected"
                self.mqtt_util.get_client_instance().publish(topic=first_device_topic, payload='true', qos=0, retain=False)
    ...
    def _publish_status(self, topic_suffix, value):
        ...
        if self.mqtt_util:
            base_topic = "OPEN-AIR/Device/Instrument_Connection/Search_and_Connect/Device_status"
            ...
            debug_log(message=f"💳 MQTT: Published status '{topic_suffix}' with value '{value}' to '{full_topic}'", **_get_log_args())
    ...
    def _publish_proxy_status(self, status: str):
        ...
        self.mqtt_util.get_client_instance().publish(topic=topic, payload=orjson.dumps(payload_data), qos=0, retain=True)
        debug_log(message=f"💳 MQTT: Published Proxy Status '{status}' to '{topic}'", **_get_log_args())

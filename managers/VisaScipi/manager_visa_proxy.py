...
class VisaProxy:
    ...
    def shutdown(self):
        if self.worker_thread and self.worker_thread.is_alive():
            debug_log(message="💳 ℹ️ Proxy Log: Shutting down VisaProxy command processor worker.", **_get_log_args())
            ...
            debug_log(message="💳 ℹ️ Proxy Log: VisaProxy worker thread not active or already shut down.", **_get_log_args())
        ...
        while not self.shutdown_flag.is_set():
            ...
            try:
                ...
                if app_constants.LOCAL_DEBUG_ENABLE:
                    debug_log(message="💳 Queue is empty, worker sleeping briefly...", **_get_log_args())
                time.sleep(0.01)
                continue
            except Exception as e:
                cmd_for_error = command_info.get("command", "N/A") if command_info is not None else "N/A"
                if app_constants.LOCAL_DEBUG_ENABLE:
                    debug_log(message=f"💳 Unhandled exception in command processor worker: {e}", **_get_log_args(), level="CRITICAL")
                self._publish_proxy_error(message=f"Error in command processor worker: {e}", command=cmd_for_error)
                if command_info is not None:
                    self.command_queue.task_done()
        debug_log(message="💳 ℹ️ Proxy Log: VisaProxy command processor worker terminated.", **_get_log_args())

    def _setup_mqtt_subscriptions(self):
        topic = "OPEN-AIR/Proxy/Tx_Inbox"
        self.subscriber_router.subscribe_to_topic(topic, self._on_tx_inbox_message)
        debug_log(message=f"💳 ℹ️ Proxy Log: 💳Subscribed to '{topic}' for inbound proxy commands.", **_get_log_args())

    def _on_tx_inbox_message(self, topic, payload):
        current_function_name = inspect.currentframe().f_code.co_name
        debug_log(message=f"💳 📡📡⬇️⬇️ PROXY IN: Tx_Inbox message received on Topic: '{topic}', Payload: '{payload}'. Proxy will process this as a raw SCPI command.", **_get_log_args())
        ...
        try:
            ...
            if command:
                ...
                debug_log(message=f"💳 ℹ️ Proxy Log: 💳Command '{command}' enqueued. Query: {query}", **_get_log_args())
            else:
    ...
    def _publish_proxy_error(self, message: str, command: str = "N/A"):
        debug_log(message=f"💳 ❌ Proxy Error: {message} (Command: {command})", **_get_log_args(), level="ERROR")

    def _publish_proxy_response(self, response: str, command: str = "N/A", correlation_id: str = "N/A"):
        debug_log(message=f"💳 📡📡⬆️⬆️ Proxy Response: {response} (Command: {command}, CorrID: {correlation_id})", **_get_log_args())
    ...
    def set_instrument_instance(self, inst):
        ...
        if self.inst:
            ...
            debug_log(message="💳 ℹ️ Proxy Log: ✅ VisaProxy is now linked to an instrument.", **_get_log_args())
            
            if self.worker_thread is None or not self.worker_thread.is_alive():
                self.shutdown_flag = threading.Event()
                self.worker_thread = threading.Thread(target=self._command_processor_worker, daemon=True)
                self.worker_thread.start()
                debug_log(message="💳 ℹ️ Proxy Log: Command processor worker thread started on connection.", **_get_log_args())
        else:
            self.shutdown()
            debug_log(message="💳 ℹ️ Proxy Log: ✅ VisaProxy has been unlinked from the instrument.", **_get_log_args(), level="WARNING")

    def _reset_device(self):
        current_function_name = inspect.currentframe().f_code.co_name
        debug_log(message=f"💳 ℹ️ Proxy Log: Attempting a system-wide reset!", **_get_log_args())
        try:
            debug_log(message="💳 ℹ️ Proxy Log: ⚠️ Command failed. Attempting to reset the instrument with '*RST'...", **_get_log_args(), level="WARNING")
            reset_success = self.write_safe(command="*RST")

            if reset_success:
                debug_log(message="💳 ℹ️ Proxy Log: ✅ Success! The device reset command was sent successfully.", **_get_log_args())
            else:

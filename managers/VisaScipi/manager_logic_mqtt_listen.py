...
class VisaMqttListener:
    ...
    def _setup_mqtt_subscriptions(self):
        try:
            ...
            debug_logger(message=f"💳 Subscribed to: {MQTT_TOPIC_SEARCH_TRIGGER}", **_get_log_args())
            self.subscriber_router.subscribe_to_topic(topic_filter=MQTT_TOPIC_DEVICE_SELECT, callback_func=self._on_device_select)
            debug_logger(message=f"💳 Subscribed to: {MQTT_TOPIC_DEVICE_SELECT}", **_get_log_args())
            ...
            debug_logger(message=f"💳 Subscribed to: {MQTT_TOPIC_CONNECT_TRIGGER}", **_get_log_args())
            ...
            debug_logger(message=f"💳 Subscribed to: {MQTT_TOPIC_DISCONNECT_TRIGGER}", **_get_log_args())
            ...
            debug_logger(message=f"💳 Subscribed to: {MQTT_TOPIC_CONNECT_RESOURCE_REQUEST}", **_get_log_args())
            debug_logger(message="💳 ✅ VisaMqttListener subscribed to all necessary GUI and command topics.", **_get_log_args())
        except Exception as e:
            debug_logger(message=f"💳 ❌ Error in VisaMqttListener._setup_mqtt_subscriptions: {e}", **_get_log_args())
    ...
    def _on_search_request(self, topic, payload):
        ...
        try:
            ...
            if payload_data.get('val') is True:
                ...
                debug_logger(message=f"💳 Ignoring Search Request, value is not 'true'. Payload Data: {orjson.dumps(payload_data)}", **_get_log_args())
        except (orjson.JSONDecodeError, AttributeError) as e:
            debug_logger(message=f"💳 ❌ Error in _on_search_request: {e}. Payload: {payload}", **_get_log_args())
        ...
        try:
            ...
            if payload_data.get('value') is True:
                ...
                if 0 <= option_index < len(self.found_resources):
                    ...
                    debug_logger(message=f"💳 ✅ Device selected: {self.selected_device_resource}", **_get_log_args())
                else:
                    self.selected_device_resource = None
            else:
                debug_logger(message=f"💳 Ignoring Device Select, value is not 'true'. Payload Data: {orjson.dumps(payload_data)}", **_get_log_args())
        except (orjson.JSONDecodeError, IndexError, ValueError, AttributeError) as e:
            debug_logger(message=f"💳 ❌ Error in _on_device_select: {e}. Payload: {payload}", **_get_log_args())
        ...
        try:
            ...
            if payload_data.get('value') is True:
                ...
                if self.selected_device_resource:
                    debug_logger(message=f"💳 🔵 Initiating connection to {self.selected_device_resource}...", **_get_log_args())
                    thread = threading.Thread(target=self._connect_and_get_inst, args=(self.selected_device_resource,), daemon=True)
                    thread.start()
                else:
                    debug_logger(message="💳 🟡 No device selected to connect.", **_get_log_args())
            else:
                debug_logger(message=f"💳 Ignoring GUI Connect Request, value is not 'true'. Payload Data: {orjson.dumps(payload_data)}", **_get_log_args())
        except (orjson.JSONDecodeError, AttributeError) as e:
            debug_logger(message=f"💳 ❌ Error in _on_gui_connect_request: {e}. Payload: {payload}", **_get_log_args())
    ...
    def _on_gui_disconnect_request(self, topic, payload):
        ...
        try:
            ...
            if payload_data.get('value') is True:
                ...
                if self.inst:
                    debug_logger(message="💳 🔵 Initiating disconnection...", **_get_log_args())
                    thread = threading.Thread(target=self.disconnector.disconnect_instrument_logic, args=(self.inst,), daemon=True)
                    thread.start()
                    self.inst = None
                else:
                    debug_logger(message="💳 🟡 No device is currently connected.", **_get_log_args())
            else:
                debug_logger(message=f"💳 Ignoring GUI Disconnect Request, value is not 'true'. Payload Data: {orjson.dumps(payload_data)}", **_get_log_args())
        except Exception as e:
            debug_logger(message=f"💳 ❌ Error in _on_gui_disconnect_request: {e}. Payload: {payload}", **_get_log_args())
    ...
    def _on_connect_request(self, topic, payload):
        ...
        try:
            ...
            if resource_name:
                debug_logger(message=f"💳 Processing Direct Connect Request for resource: {resource_name}. Payload Data: {orjson.dumps(payload_data)}", **_get_log_args())
                thread = threading.Thread(target=self._connect_and_get_inst, args=(resource_name,), daemon=True)
                thread.start()
            else:
                debug_logger(message=f"💳 Ignoring Direct Connect Request, no resource_name in payload. Payload Data: {orjson.dumps(payload_data)}", **_get_log_args())
        except orjson.JSONDecodeError:
            debug_logger(message=f"💳 ❌ Failed to decode JSON payload for connect request: {payload}", **_get_log_args())
        except Exception as e:
            debug_logger(message=f"💳 ❌ Error in _on_connect_request: {e}. Payload: {payload}", **_get_log_args())

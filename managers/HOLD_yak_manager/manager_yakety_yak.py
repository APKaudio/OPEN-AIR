...
class YaketyYakManager:
    ...
    def __init__(self, mqtt_controller: MqttControllerUtility, dispatcher_instance: ScpiDispatcher, app_instance):
        ...
        try:
            ...
            debug_log(message="✅ The YaketyYak Manager has initialized and is listening for commands.", **_get_log_args())
    ...
    def YAK_LISTEN_TO_MQTT(self, topic, payload):
        ...
        try:
            ...
            debug_log(message=f"✅ Repository updated in memory and saved to file from message on topic: {topic}", **_get_log_args())
    ...
    def YAK_SAVE_REPOSITORY(self, topic, payload):
        ...
        try:
            ...
            debug_log(message=f"❌ Error saving repository to file: {e}", **_get_log_args())
            if app_constants.LOCAL_DEBUG_ENABLE: 
                debug_log(
                    message=f"🐐🐐🐐🔴 The quill broke! Failed to save the repository! The error be: {e}",
                    **_get_log_args()

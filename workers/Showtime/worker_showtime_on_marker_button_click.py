...
def on_marker_button_click(showtime_tab_instance, button):
    ...
    if showtime_tab_instance.selected_device_button == button:
        ...
        debug_logger(message=f"🟡 Deselected device: {marker_data.get('NAME', 'N/A')}.", **_get_log_args())
    else:
        ...
        debug_logger(message=f"✅ Selected device: {marker_data.get('NAME', 'N/A')} at {marker_data.get('FREQ_MHZ', 'N/A')} MHz.", **_get_log_args())

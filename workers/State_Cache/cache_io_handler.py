...
def load_cache() -> Dict[str, Any]:
    ...
    """
    debug_logger(message="Great Scott! We're attempting to read the Almanac!", **_get_log_args())
    try:
        if app_constants.DEVICE_STATE_SNAPSHOT_PATH.exists():
            ...
            with open(app_constants.DEVICE_STATE_SNAPSHOT_PATH, 'rb') as f:
                ...
                debug_logger(message="Great Scott! The timeline has been successfully loaded from the Almanac!", **_get_log_args())
                return data
        else:
            debug_logger(message="The Almanac is blank! No cache file found.", **_get_log_args())
    except Exception as e:
        debug_logger(message=f"Great Scott! The Almanac is unreadable! Could not read the cache file: {e}", **_get_log_args())
    return {}
...
def save_cache(data: Dict[str, Any]) -> bool:
    ...
    """
    debug_logger(message="Great Scott! We're about to write to the Almanac!", **_get_log_args())
    try:
        ...
        debug_logger(message=f"Temporal duplicate created at {temp_path}. Now, for the switch!", **_get_log_args())

        os.rename(temp_path, app_constants.DEVICE_STATE_SNAPSHOT_PATH)
        debug_logger(message="Great Scott! The timeline has been successfully recorded in the Almanac!", **_get_log_args())
        return True
    except Exception as e:
        debug_logger(message=f"Great Scott! We've created a paradox! Failed to save the cache: {e}", **_get_log_args())

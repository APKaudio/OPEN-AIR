...
class FleetStatusMonitor:
    ...
    def __init__(self, state_mirror_engine, subscriber_router: MqttSubscriberRouter):
        ...
        self.subscriber_router.subscribe_to_topic(f"{self.base_topic}/Start", self._on_scan_start)
        # 'Complete' comes from the Visa Manager finishing
        self.subscriber_router.subscribe_to_topic(f"{self.base_topic}/Complete", self._on_scan_complete)
    ...
    def _on_scan_complete(self, topic, payload):
        try:
            ...
            if num_devices > 0:
                ...
                debug_logger(f"🟢 Fleet Scan Complete - {num_devices} devices found. Status Green")
            else:
                self.current_state = "RED"
                self._publish_color("red")
